...
cd -
#public_IP=`../tools/get_public_ip.py`
#if [ "`../tools/get_ip_area.py $public_IP`" == 'CN' ];then
	if [ -n "$(cat /etc/redhat-release | grep '6\.')" ];then
		#wget -c http://soft.hhvm.biz/centOS/CentOS6-Base.repo -P /etc/yum.repos.d
		...
		#wget -c http://soft.hhvm.biz/centOS/CentOS5-Base.repo  -P /etc/yum.repos.d
		if [ ! -z "$(cat /etc/redhat-release | grep 'Red Hat')" ];then
	        	/bin/mv /etc/yum.repos.d/CentOS-Base.repo{,_bk}
